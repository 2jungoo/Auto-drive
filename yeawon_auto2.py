...
class DriveMode(Enum):
    AR_CURVE_CONTROL = "AR_CURVE_CONTROL" # 기존 LANE_FOLLOWING을 AR_CURVE_CONTROL로 변경
    RUBBERCON_AVOIDANCE = "RUBBERCON_AVOID"
    EMERGENCY_STOP = "EMERGENCY_STOP"

class WebViewer(BaseHTTPRequestHandler):
    # (이 부분은 기존 코드와 동일합니다. 변경 사항 없음.)
...
class Autoracer(Node):
    def __init__(self):
        ...
        # 미션 상태 관리
        self.current_mode = DriveMode.AR_CURVE_CONTROL # AR_CURVE_CONTROL 모드로 시작
    ...
    def find_rubbercons(self, image):
        ...
        for contour in contours:
            ...
            if area > 200:
                ...
                if distance_from_bottom < 150: # 특정 거리(예: 1.5m) 이내 라바콘만 감지
    ...
    def detect_and_avoid_rubbercon_enhanced(self, image):
        """요청하신 메커니즘에 따라 라바콘 회피 로직 수정"""
        height, width = image.shape[:2]
        
        rubbercons = self.find_rubbercons(image)

        # 1. 특정 거리 이내에서 장애물 인식시 flag = 1
        if len(rubbercons) > 0:
            ...
            if left_cones and right_cones:
                ...
                # 양쪽 라바콘 사이의 중앙으로 목표 설정
                target_x = (left_most['center_x'] + right_most['center_x']) // 2
                
                # 오차값 중첩: 양쪽 라바콘과의 거리 오차를 계산하여 조향각 보정
        ...
        # 2. flag = 1 이면서 장애물 인식 안 됐을 때 모드 종료
