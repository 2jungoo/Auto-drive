...
class Autoracer(Node):
    ...
    def calc_steering_angle(self, lane_center_bev):
        ...
        kp_dynamic = 0.001 * (1.0 + abs(offset) / (self.proc_w / 4.0))
        curve_bias = 0.0
    ...
    def convert_lidar_to_image_coords(self, lidar_point):
        # LiDAR 좌표를 이미지 좌표로 변환하는 로직 (단순화된 예시)
    ...
    def control_loop(self):
        ...
        try:
            ...
            if not self.started:
                ...
                if self.traffic_light_checked_once and traffic_light_state == "RED":
                    # 이 로직은 단 한 번만 감지하는 방식에서는 작동하지 않음
                    # 필요시 신호등 재검출 로직을 추가해야 함
                    pass
                ...
                if self.cone_mode_active:
                    ...
                    if path_center_lidar is not None:
                        ...
                        steering = self.calc_steering_angle(path_center_pixels[0]) # x 좌표만 사용
                    ...
                    if (self.lock_rem > 0) and (self.last_lane_center is not None) and (not force_redetect):
                        ...
                        self.prev_left_fit, self.prev_right_fit, self.prev_yellow_fit = lf, rf, yf
            ...
            if (self.frame_idx % self.DEBUG_EVERY) == 0:
                dbg = self.image.copy() if self.PROC_SCALE == 1.0 else cv2.resize(
                    self.image, (self.proc_w, self.proc_h), interpolation=cv2.INTER_AREA)
                vis = dbg.copy()
                if self.started:
                ...
                cv2.putText(vis, f"Status: {'Driving' if self.started else 'Waiting'} | Speed: {speed_mps:.2f}",
                            (10,30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,255), 2)
                cv2.putText(vis, f"mode={'CONE_AVOID' if self.cone_mode_active else src_used} | steer={math.degrees(steering):.1f}deg",
                            (10,60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,255), 2)
