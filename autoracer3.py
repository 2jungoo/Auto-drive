...
class WebViewer(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/':
            ...
            <body>
                <h1>🏆 Autoracer 2025 Contest - Continuous Driving</h1>
                <div class="container">
                    ...
                    <div class="panel" style="flex: 1;">
                        ...
                        <div id="alerts" style="background: rgba(0,255,0,0.1); border-radius: 8px; padding: 15px; min-height: 50px;">
                            <span style="color: #00ff88;">Continuous Autonomous Driving Active</span>
                        </div>
                    </div>
                </div>
                ...
                setInterval(() => {
                    ...
                    .then(data => {
                        ...
                        // Status - always show continuous driving
...
class Autoracer(Node):
    def __init__(self):
        ...
        self.get_logger().info('🏆 Autoracer 2025 Contest Started - Continuous Driving Mode!')
    ...
    def draw_status_header(self, image):
        ...
        # 텍스트 정보
        cv2.putText(image, f'🏆 Continuous Mode | Mode: {self.current_mode.value}', 
    ...
    def calculate_steering_control(self, error, mode):
        ...
        if mode == "RUBBERCON":
            ...
            if steering_magnitude < 0.1:  # 직선 구간
                base_speed = 0.5  # 고속 주행, 원래 0.7
            else:  # 곡선 구간
                base_speed = 0.25 # 수정 , 원래 0.5
        else:
            base_speed = 0.2
    ...
    def draw_lidar_overlay(self, image):
        ...
        if valid_ranges:
            ...
            # 거리에 따른 색상 및 상태 (정보만 표시, 제어에 영향 없음)
            ...
            cv2.putText(image, f"Min Distance: {min_distance:.2f}m", (panel_x + 10, panel_y + 70), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
            cv2.putText(image, status + " (INFO ONLY)", (panel_x + 10, panel_y + 95), 
    ...
    def lidar_callback(self, msg):
        """라이다 콜백 - 정보만 수집, 제어에 영향 없음"""
        self.lidar_data = msg
        # 응급정지 로직 제거 - 라이다 데이터는 정보 표시용으로만 사용
